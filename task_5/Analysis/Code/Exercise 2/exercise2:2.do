...
//This is the do file for Exercise 2/2(Out of sample prediction) in Task 5. In this exercise, we are using 1992-2002 growth dataset. The goal is to use our estimated predictors from the 1992-2002 dataset to predict growth in 2002-2011.
...
* Random forest
	...
	regress growth presidential inflation fem_emp corruption parliamentary effectiveness lexp 	regulation if wave == 2
	regress growth presidential inflation fem_emp corruption parliamentary effectiveness lexp regulation if wave == 1
	predict growth_hat_rforest, xb
	egen mse_rforest= total((growth_hat_rforest - growth)^2), by(wave)
